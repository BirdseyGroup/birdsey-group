...
### SEO indexability switch (launch checklist item)
`lib/seo.ts` exports `IS_INDEXABLE`: robots.txt disallows everything and pages get a noindex meta unless the deployment's `VERCEL_PROJECT_PRODUCTION_URL` contains `birdseygroup.com` (or `NEXT_PUBLIC_SITE_INDEXABLE=true` is set). Staging/preview projects are therefore never indexed. **At launch, after attaching the real domain, verify `/robots.txt` on production allows crawling** — if Vercel system env vars are disabled on the project, set `NEXT_PUBLIC_SITE_INDEXABLE=true` manually. Related endpoints: `/sitemap.xml` ([app/sitemap.ts](app/sitemap.ts), includes insights, enabled team profiles, custom pages) and `/llms.txt` ([app/llms.txt/route.ts](app/llms.txt/route.ts), generated from CMS content). JSON-LD: Organization (layout), Article (insights), Person (team profiles).

### Gotcha: tina-lock.json must be regenerated for every tina/config.ts change
Deployed builds compare a hash of the CI-compiled schema against Tina Cloud's hash of the committed `tina-lock.json`. Even a ui-only config change (e.g. adding `ui.optionComponent`, which compiles to an empty `"ui": {}` marker) changes the compiled schema, so committing config without the regenerated lock makes **every deploy fail** with `ERR_CLOUD_CHECK_FAILED` ("local Tina schema doesn't match the remote") until the lock is fixed. The lock is only rewritten by a running `tinacms dev`; `tinacms build --local` does NOT touch it. Before committing any `tina/config.ts` change, make sure `npm run dev` has picked it up and include the resulting `tina-lock.json` diff in the same commit.

### Gotcha: SDS global CSS vs CSS-module specificity ties
SDS primitives (Navigation, NavigationPill, Section, ...) attach global classes (`.navigation-pill`, `.section-variant-brand`) alongside any module class you pass. A bare single-class module rule ties those global rules on specificity, so the winner is decided by CSS chunk order, which **differs between dev and production bundles** (dev looked right; deployed builds showed the desktop nav on mobile and dark mobile menu links). Never rely on order: scope module rules under a parent class (`.header .mobileNavigationPill`) or double the class (`.desktopNav.desktopNav`, `.header.header`), and restate properties the global rule sets on stateful selectors like `[data-selected]`.
